...
public class Type extends DelegatingTree<Definition> implements IType {
	...
	/*
	 * Fleeting classes cannot be used in other classes, only instantiated. On the
	 * other hand, they can be re-compiled and re-deployed to the Virtual Machine
	 * (in which case, all instances of the class are destroyed).<br> Pure is the
	 * synonym for abstract class (abstract is just a reserved word)
	 */
	...
	public static Type prototype(String packageName, String simpleName, Integer lang, String sourceCode) {
	...
	public Type(TypeName name, Integer lang, TypeName base) {
		...
		add(BaseConstructor.class, new StdMultiTree<>(BaseConstructor.class, BaseConstructor::sameSignatureAs));
	}
		...
		for (Entry<String, Definition> e : this) {
			...
			if (d instanceof BaseMethod) {
				...
				logger.trace("Collecting imports in the {} {}",
						m.getMethodType() == MethodType.METHOD ? "method" : "constructor", m.getName());
