...
public class JType extends TranslatedType {
	...
	public void setCompiled(Class<?> jClass) {
		if (this.jClass != null) {
			throw new IllegalArgumentException("Java compiled class is already set for '" + getFullName() + "'");
		}
		if (!jClass.getCanonicalName().equals(getFullName())) {
			throw new IllegalArgumentException("Java Class' canonical name '" + jClass.getCanonicalName()
					+ "' is not equal to the type's full name '" + getFullName() + "'");
		}
	...
	private void updateType(Class<?> source) {
		try {
			...
			for (Method m : source.getDeclaredMethods()) {
				...
				BaseMethod m2 = JavaRefHelper.methodFromJava(m.getName(), m.getModifiers(), m.getParameters(),
						m.getGenericParameterTypes(), m.getReturnType());
			...
			if (!notUpdated.isEmpty()) {
				String message = "Following methods have not been found in the Java-Class '" + source.getCanonicalName()
			...
			for (Constructor<?> javaC : source.getDeclaredConstructors()) {
				...
				BaseMethod baseM = JavaRefHelper.methodFromJava(source.getSimpleName(), javaC.getModifiers(),
						javaC.getParameters(), javaC.getGenericParameterTypes(), source);
				baseM.setMethodType(MethodType.CONSTRUCTOR);
				JavaConstructor constr = new JavaConstructor(baseM, javaC);
				if (stdConstr == null || baseM.getArguments().size() > stdConstr.getArguments().size()) {
					stdConstr = constr;
				}
			...
			if (getStdConstructor() == null) {
				if (stdConstr == null && !isAbstract()) {
					throw new IllegalArgumentException("Class '" + source.getSimpleName() + "' has no constructors");
			...
			if (!notUpdated.isEmpty()) {
				throw new IllegalArgumentException("Following constructors have not been found in the Java-Class '"
						+ source.getCanonicalName() + "': " + notUpdated);
			}
