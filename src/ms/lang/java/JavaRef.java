...
public class JavaRef extends DelegatingTree<Instance<Object>> implements Instance<Object> {
    ...
    private static <T> Class<Instance<T>> getInstanceClass() {
        return (Class) Instance.class;
    }

    // Java Class Ref
    public JavaRef(Class<?> staticClass) {
        super(getInstanceClass());
    ...
    private void create(Object instObj, Class<?> instClass, boolean onlyStatic) {
        Iterable<Field> fit
                = toIterable(() -> array(new ClassIterator(instClass), Class::getDeclaredFields));
        Function<String, Field> retriever = s -> first(fit, f -> f.getName().equals(s));
        Function<Field, String> idRetriever = Field::getName;
        Function<Field, Instance<Object>> converter
                = f -> new JavaRef(getValue(instObj, f), f.getGenericType());
        BiConsumer<Field, Instance<Object>> setter = (f, o) -> setValue(instObj, f, o.getValue());

        Iterable<Class<?>> fit1
                = toIterable(() -> array(new ClassIterator(instClass), Class::getDeclaredClasses));
        ...
        BiConsumer<Class<?>, Instance<Object>> setter1
                = (c, o) -> error("Cannot set nested member class " + c.getCanonicalName());

        SyntheticTree<Instance<Object>> memberEntity = new SyntheticTree<>(converter,
                                                                           setter,
                                                                           idRetriever,
                                                                           retriever,
                                                                           fit,
                                                                           getInstanceClass());
        memberEntity.setDeleter(s -> error("Cannot delete member '" + s + "' in instance of type '"
                                           + instClass.getCanonicalName() + "'"));

        SyntheticTree<Instance<Object>> typeEntity = new SyntheticTree<>(converter1,
                                                                         setter1,
                                                                         idRetriever1,
                                                                         retriever1,
                                                                         fit1,
                                                                         getInstanceClass());
        typeEntity.setDeleter(s -> error("Cannot delete nested class '" + s + "' in '"
                                         + instClass.getCanonicalName() + "'"));
    ...
    private static void checkField(Object parent, Field field) {
        ...
        if (parent == null && !Modifier.isStatic(field.getModifiers())) {
            throw new IllegalArgumentException("Cannot use non-static member object '"
                                               + field.getName() + "' if the parent is null");
        }
    }
    ...
    private static Object getValue(Object parent, Field field) {
        ...
        try {
            ...
            throw new IllegalArgumentException("System-design-fault: illegal argument|access exception is inacceptable");
        }
    }
    ...
    static TypeName fromClass(Class<?> jClass) {
        ...
        return jClass == null ? null
                : new TypeName(jClass.getSimpleName(), jClass.getCanonicalName(), false);
    }
    ...
    static TypeName fromReflectType(java.lang.reflect.Type type) {
        ...
        if (generic) {
            ...
            return new TypeName(StdPathManipulator.toSimpleName(type.getTypeName()),
                                type.getTypeName(),
                                false);
        }
    }
    ...
    static Class<?> fromType(Type type) {
        ...
        throw new IllegalArgumentException("Type " + type + " is of unknown class "
                                           + type.getClass());
    }
