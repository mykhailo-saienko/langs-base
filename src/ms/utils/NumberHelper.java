...
public class NumberHelper {
    ...
    public static synchronized DecimalFormat getQtyFormat() {
        if (qtyFormat == null) {
            qtyFormat
                    = new DecimalFormat("#,##0", new DecimalFormatSymbols(DateHelper.getLocale()));
        }
        return qtyFormat;
    }
    ...
    public static BigDecimal bd(double val, int scale) {
        try {
            ...
            logger.fatal("An error occured while converting double into BigDecimal. This is a design issue",
                         e);
            return null;
        }
    }
    ...
    /**
     * A sufficiently precise computation routine for square root which uses the Babylonian method.
    ...
    /**
     * Compares two {@link BigDecimal} numbers. NOTE: two numbers with the same values but different
     * scales, e.g. 2.0 and 2.00, are considered equal by this method.
     * 
     * @param arg0
     * @param arg1
     * @return
    ...
    /**
     * Returns true if the two {@link BigDecimal} numbers are equal (ignoring their scale).
     * 
     * @param arg0
     * @param arg1
     * @return
